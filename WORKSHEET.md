...
**تاریخ: 2024-08-15**
*   **تسک:** رفع باگ بحرانی و افزایش پایداری سیستم
*   **توضیحات:**
    1.  **رفع خطای Uncaught TypeError:** یک باگ بحرانی که باعث از کار افتادن برنامه هنگام دسترسی به فایل‌های حذف شده می‌شد (خطای `Cannot read properties of undefined (reading 'name')`) به طور کامل برطرف شد.
    2.  **افزایش پایداری:** چندین بخش کلیدی برنامه, از جمله نمایش فایل‌ها, پردازش فایل, گزارش جامع و ماژول چاپ, بازبینی و مقاوم‌سازی شدند. اکنون سیستم به درستی شرایطی را مدیریت می‌کند که داده‌ها در پس‌زمینه (مثلاً توسط کاربر دیگر) تغییر کرده باشند. این بهبود از بروز خطاهای مشابه در آینده جلوگیری کرده و پایداری کلی داشبورد را به میزان قابل توجهی افزایش می‌دهد.

**تاریخ: 2024-08-14**
*   **تسک:** رفع مشکلات رابط کاربری و منطق تخصیص کد بچ
*   **توضیحات:**
    1.  **رفع مشکل بسته شدن منوی جزئیات:**
        *   یک باگ آزاردهنده در ماژول "رهگیری تولید" رفع شد که باعث می‌شد منوی "جزئیات" با هر بار تیک زدن یک مرحله تولید، به صورت خودکار بسته شود. اکنون منو باز باقی می‌ماند تا کاربر بتواند به راحتی تمام مراحل را مدیریت کند.
    2.  **بهبود چیدمان جزئیات بچ:**
        *   رابط کاربری بخش "مصرف مواد اولیه" در منوی جزئیات رهگیری تولید بازبینی و بهینه شد. چیدمان جدید خواناتر است و بخش‌های مواد استاندارد و خرج کار دستی به صورت منظم‌تر نمایش داده می‌شوند.
    3.  **اصلاح منطق تخصیص کد بچ گروهی:**
        *   یک خطای مهم در منطق تخصیص کد بچ برطرف شد. پیش از این، هنگام انتخاب چند گروه محصول مختلف، یک کد بچ یکسان به همه آنها اختصاص داده می‌شد. اکنون سیستم به درستی برای هر گروه محصول یک کد بچ منحصر به فرد و مجزا بر اساس الگوی تعریف شده (پیشوند A یا K) تولید می‌کند.

**تاریخ: 2024-08-13**
*   **تسک:** رفع باگ و بازبینی مراحل تولید
*   **توضیحات:**
    1.  **رفع باگ بحرانی در رهگیری تولید:**
        *   یک باگ مهم که باعث از کار افتادن ماژول "رهگیری تولید" (خطای `reduce`) به دلیل داده‌های قدیمی یا ناسازگار می‌شد، برطرف گردید. سیستم اکنون در برابر این نوع خطاها مقاوم است و لیست بچ‌ها همیشه به درستی نمایش داده می‌شود.
    2.  **بازبینی و به‌روزرسانی مراحل تولید:**
        *   بنا به درخواست کاربر، لیست مراحل تولید در بخش جزئیات بچ در ماژول "رهگیری تولید" بازبینی و به‌روزرسانی شد.
        *   مراحل جدید عبارتند از: `آماده سازی و الگو`, `برش`, `دوخت`, `اتو`, `کنترل کیفی`, `برون سپاری`, `سنگ شوری`, و `بسته بندی`. این تغییر به کاربران امکان می‌دهد تا فرآیند تولید را با دقت بیشتری مطابق با گردش کار واقعی خود رهگیری کنند.
